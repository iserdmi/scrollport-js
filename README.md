# Scrollport.js
Scrollport.js — не скучный плагин для анимации скролла. Примитивное зрелище плавного передвижения скролла, которое мы видим на многих сайтах, всео лишь 1 из 4 режимов в этом плагине. Посмотрите [демо](http://serdmi.com/demo/scrollport/) и возвращайтесь за информацией о подключении и пользовании.
...
При нажатии на созданную ссылку будет инициированна работа плагина. Все ссылки с атрибутом `data-scrollport` автоматически будут считаться ссылками скроллпорта. Если не передать ни одно из значений `target`, `top` или `left`, тогда `target` автоматически примет значение аттрибута `data-scrollport` или `href`, или `data-href`.
...
* **`api.status`**  
В момент инициализации статус `init`. Если произошел сбой при инициализации, или инциализация была отменена значение будет `cancel`. Во время движения `motion`. После прерывания `interrupt`. При успешном завершении `finish`.
